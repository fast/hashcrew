...
CI only checks that benchmarks compile; it does not run performance measurements or enforce timing thresholds. Both stable and MSRV CI use `cargo x build --locked` to compile all workspace targets with every Hashcrew family and comparison enabled, including both benchmark executables. Use the same command locally when only a build check is needed. `cargo x test` runs the workspace tests on either toolchain.
